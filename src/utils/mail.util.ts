...
export const sendEmail = async (sending_info: sending_info) => {
...
export const sendVerificationEmail = async (email: string, token: string) => {
	let sending_info = {
		...
		text:
			`Verification email. Please verify your account through this link http://localhost:${process.env.PORT}/verify/` +
			commonUtils.normalizeVerifyToken(token),
		html:
			`<h2>Verification email</h2><a href="localhost:${process.env.PORT}/verify/` +
			commonUtils.normalizeVerifyToken(token) +
...
export const sendResetPwdEmail = async (email: string, token: string) => {
	let sending_info = {
		send_to: email,
		subject: "Reset your password",
		text:
			`To reset your password and set up a new one. Please access this link http://localhost:${process.env.PORT}/reset_pwd/` +
			commonUtils.normalizeVerifyToken(token),
		html:
			`<h2>Reset your password here </h2><a href="localhost:${process.env.PORT}/reset_pwd/` +
			commonUtils.normalizeVerifyToken(token) +
