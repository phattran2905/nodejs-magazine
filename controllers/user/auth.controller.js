...
module.exports = {
  ...
  showVerifyTokenForm: async (req, res) => {
    ...
    if (isValidToken) {
      try {
        const accountAuthor = await authorUtils.verifyAccountByToken(
          req.params.verify_token
        )
        if (accountAuthor) {
          req.flash(
            'verifySuccess',
            'Successfully. Your account is verified. You can log in now!'
          )
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (verification) {
          req.flash(
            'sendSuccess',
            'Successfully. Please check your email to verify your registered account. '
          )
          return res.render('user/auth/send_email', {
            form: {
              title: 'Verify account',
  ...
  sendResetPwdEmail: [
    ...
    async (req, res) => {
      ...
      if (isPending) {
        req.flash(
          'sendSuccess',
          'An email was already sent to you. Please check your email again.'
        )
      ...
      try {
        ...
        if (resetPwdRes) {
          req.flash(
            'sendSuccess',
            'Successfully. Please check your email to reset your password'
          )
          return res.render('user/auth/send_email', {
            form: {
              title: 'Reset Password',
  ...
  resetPwd: [
    ...
    async (req, res) => {
      ...
      if (isValidToken) {
        ...
        const resetPwdRes = await authorUtils.reset_pwd(
          req.params.verify_token,
          validInput.password
        )
