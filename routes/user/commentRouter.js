...
module.exports = (userRouter) => {
    userRouter.post(
        ...
        async(req, res) => {
            try {
                const article = await ArticleModel.findOne({$and: [{status: 'Published', _id: req.params.articleId}] });
                if (article){
    ...
    userRouter.post(
        '/comment/:interaction/:commentId',
        async(req, res) => {
            if(req.params.interaction === 'like' || req.params.interaction === 'dislike') {
                const interaction = req.params.interaction;
                try {
                    let comment = await CommentModel.findOne({$and: [{status: 'Public', _id: req.params.commentId}] });
                    if (comment) {
                        if (interaction == 'like') { comment.likes += 1 }
                        else if (interaction == 'dislike') { comment.dislikes += 1 };
                        await comment.save();
